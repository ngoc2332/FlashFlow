...
import { EventEnvelope, HeaderValue, headerToString, parseEventEnvelope } from "@flashflow/common";
import { resolveOrderStatus } from "../domain/order-status-policy";
import { UpsertResult } from "../infrastructure/order-status-view-repository";
...
export interface ProcessStatusMessageInput {
  ...
  upsertOrderStatus: (event: EventEnvelope<unknown>, status: string) => Promise<UpsertResult>;
...
export async function processStatusMessageUseCase(
  input: ProcessStatusMessageInput,
): Promise<void> {
  ...
  if (!input.primaryTopics.includes(input.payload.topic) && targetWorker !== input.serviceName) {
  ...
  try {
    event = parseEventEnvelope(input.payload.message.value);
    ...
    const status = resolveOrderStatus(event.eventType);
    ...
    const upsertResult = await input.upsertOrderStatus(event, status);

    if (upsertResult.applied) {
      await input.publishStatusSnapshot(
        event,
